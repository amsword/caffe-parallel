...
void CCCPLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    if (bias_term_) {
      caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, group_ * num_output_,
...
void CCCPLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (bias_term_) {
    ...
    for (int n = 0; n < num_; ++n) {
      caffe_gpu_gemv<Dtype>(CblasNoTrans, num_output_ * group_,
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    for (int g = 0; g < group_; ++g) {
      ...
      caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasTrans, num_output_,
