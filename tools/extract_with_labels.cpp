  ...
  for (int batch_index = 0; batch_index < num_mini_batches; ++batch_index) {
    ...
    for (int i = 0; i < num_features; ++i) {
      ...
      const shared_ptr<Blob<Dtype> > label_blob = feature_extraction_net
          ->blob_by_name(label_name);

      int batch_size = feature_blob->num();
      int dim_features = feature_blob->count() / batch_size;
      int dim_labels = label_blob->count() / batch_size;
      ...
      for (int n = 0; n < batch_size; ++n) {
        ...
        const Dtype* label_blob_data = label_blob->cpu_data() + label_blob->offset(n);
        if (dim_labels == 1) {
            datum.set_label(label_blob_data[0]);
        } else {
            datum.clear_labels();
            for (int d = 0; d < dim_labels; ++d) {
                datum.add_labels(label_blob_data[d]);
            }
        }
        string value;
